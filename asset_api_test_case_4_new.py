...
class Oracle(aggregator_client_type):
    ...
    def on_event_or_response(self, message):
        if "trade_list" in message:
            ...
            current_trades2 = api_util.search_trade_history(message['trade_list'], ev_2_devices)
            self.market_slot_trades2.extend(current_trades2)
            current_trades3 = api_util.search_trade_history(message['trade_list'], ev_3_devices)
            self.market_slot_trades3.extend(current_trades3)
            current_trades4 = api_util.search_trade_history(message['trade_list'], ev_4_devices)
            self.market_slot_trades4.extend(current_trades4)
    ...
    def on_finish(self, finish_info):
        ...
        model2.save('bachelor_thesis/model_ev_2')
        rew_ev_2 = pd.DataFrame(env_ev_2.reward_list, columns=['reward'])
        rew_ev_2.to_csv('bachelor_thesis/ev_2_results.csv')
        model3.save('bachelor_thesis/model_ev_3')
        rew_ev_3 = pd.DataFrame(env_ev_3.reward_list, columns=['reward'])
        rew_ev_3.to_csv('bachelor_thesis/ev_3_results.csv')
        model4.save('bachelor_thesis/model_ev_4')
        rew_ev_4 = pd.DataFrame(env_ev_4.reward_list, columns=['reward'])
        rew_ev_4.to_csv('bachelor_thesis/ev_4_results.csv')
...
model2 = A2C('MlpPolicy', env_ev_2, verbose=1)
model3 = A2C('MlpPolicy', env_ev_3, verbose=1)
model4 = A2C('MlpPolicy', env_ev_4, verbose=1)
